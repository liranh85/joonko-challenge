...
describe('/users', () => {
  ...
  describe('/jobs', () => {
    it('should return 200 and RnD jobs for first user', () => {
      const RND_JOBS = OPEN_JOBS.filter(
        (job) => job.department === DEPARTMENTS.RND
      );

      return request
        .get(`${API_PREFIX}/users/jobs`)
        .set('Cookie', [
          '_user_session=j%3A%7B%22email%22%3A%22a%40joonko.co%22%7D',
        ])
        .expect(200)
        .then((response) => {
    ...
    it('should return 200 and marketing jobs for second user', () => {
      const MARKETING_JOBS = OPEN_JOBS.filter(
        (job) => job.department === DEPARTMENTS.MARKETING
      );

      return request
        .get(`${API_PREFIX}/users/jobs`)
        .set('Cookie', [
          '_user_session=j%3A%7B%22email%22%3A%22b%40joonko.co%22%7D',
        ])
        .expect(200)
        .then((response) => {
          const { jobs } = response.body;
    ...
    it('should return 200 and product jobs for third user', () => {
      const PRODUCT_JOBS = OPEN_JOBS.filter(
        (job) => job.department === DEPARTMENTS.PRODUCT
      );
      ...
      return request
        ...
        .expect(200)
        .set('Cookie', [
          '_user_session=j%3A%7B%22email%22%3A%22c%40joonko.co%22%7D',
        ])
        .then((response) => {
          const { jobs } = response.body;
    ...
    it('should return 200 and all jobs for forth user', () => {
      return request
        ...
        .expect(200)
        .set('Cookie', [
          '_user_session=j%3A%7B%22email%22%3A%22d%40joonko.co%22%7D',
        ])
        .then((response) => {
          const { jobs } = response.body;
    ...
    it('should return 401 for unauthorized user', () => {
      return request.get(`${API_PREFIX}/users/jobs`).expect(401);
    });
  });
});
